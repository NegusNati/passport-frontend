...
import { type AdminArticleCreatePayload, AdminArticleCreateSchema } from '../schemas/create'
...
export async function createAdminArticle(input: AdminArticleCreatePayload) {
  ...
  try {
    ...
    throw new Error(
      extractAdminArticleErrorMessage(
        error,
        'Failed to create article. Please review the form and try again.',
      ),
    )
  }
}
